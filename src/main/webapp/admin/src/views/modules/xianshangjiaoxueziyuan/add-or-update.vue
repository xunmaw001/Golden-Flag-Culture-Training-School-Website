    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="视频详细" prop="shipinxiangxi">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.shipinxiangxi" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shipinxiangxi" label="视频详细" prop="shipinxiangxi">
                    <span v-html="ruleForm.shipinxiangxi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shipinjieshao = true;
          ...
          if(o=='shipinxiangxi'){
            this.ruleForm.shipinxiangxi = obj[o];
	    this.ro.shipinxiangxi = true;
            continue;
          }
        }
      }
    ...
    info(id) {
      this.$http({
        url: `xianshangjiaoxueziyuan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.shipinxiangxi = this.ruleForm.shipinxiangxi.replace(reg,'../../../ssmvayj3/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xianshangjiaoxueziyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xianshangjiaoxueziyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
