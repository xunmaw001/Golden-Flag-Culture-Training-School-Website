    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="课程详细" prop="kechengxiangxi">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.kechengxiangxi" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.kechengxiangxi" label="课程详细" prop="kechengxiangxi">
                    <span v-html="ruleForm.kechengxiangxi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kechengjieshao = true;
          ...
          if(o=='kechengxiangxi'){
            this.ruleForm.kechengxiangxi = obj[o];
	    this.ro.kechengxiangxi = true;
            continue;
          }
        }
      }
	...
	this.ruleForm.kechengxiangxi = this.ruleForm.kechengxiangxi.replace(reg,'../../../ssmvayj3/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `kechengjieshao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.kechengjieshaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
